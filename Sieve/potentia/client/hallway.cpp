...
const char* kUsage =
    ...
    "Lines (the same meaning as in the potentia tool):\n"
    ...
    "  --key K             scramble key (default potentia)\n"
...
std::vector<Line> make_lines(const Args& a)
{
    ...
    for (LineKind k : kLineOrder)
    {
        ...
        la.opts["key"] = a.get("key", "potentia");
        switch (k)
        {
        case LineKind::Text:
...
int main(int argc, char** argv)
{
    try
    {
        // Reuse the potentia argument parser: it expects a command first.
