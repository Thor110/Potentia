# Potentia — Gallery of Babel core
...
| `tools/potentia_cli.cpp`, `tools/cli/` | The `potentia` command-line tool |
...
| `reference/potentia_ref.py` | Independent Python oracle; generates every conformance vector file |
...
The toolchain matches ALTEngine: CMake, with a vcpkg manifest. The core and the `potentia` tool have **no dependencies**. The `hallway` needs SDL3:
...
- To build without the hallway, configure with `-DPOTENTIA_BUILD_CLIENT=OFF`.
...
**Continuous integration:** `.github/workflows/build.yml` builds and tests on Windows, Linux and macOS. It checks that every platform produces the same addresses, and that the Python oracle reproduces the committed vectors. GitHub only reads workflows from the repository root: if this folder is a subfolder of your repository (as in `Potentia/potentia`), move `.github` up to the repository root. `PROJECT_DIR` in the file is already set to `potentia`.
...
```sh
potentia                  # commands and a quick start
potentia help warp        # full explanation and examples for one command (or: potentia warp --help)
potentia help lines       # the four lines and all of their options
```
...
Every line also takes `--key K` (default `potentia`), which seeds the scrambled ordering.
...
**Options.** The hallway takes the same line options as `potentia`, renamed per line because all four lines are open at once:
...
```sh
potentia info --length 1000                              # paragraph-scale text
potentia info --line image --palette rgb24               # every 10x10 full-colour image
potentia info --line video                               # every 8-frame 5x5 animation
```
...
potentia warp --length 1000 --file chapter1.txt
potentia warp --line image --file sprite.png
potentia warp --line image --palette ega16 --width 16 --height 16 --file sprite.png
potentia warp --line audio "E4q D4q C4q D4q E4q E4q E4h"
potentia warp --line video --file walk.gif --short
```
...
```
line         text
space        lower27/L32/key=potentia/feistel-sha256-v1
...
unit 1/1  "it was the best of times        "
  ...
  scrambled   0ec971d8ace67261044340a270bfadb62f567ec
              at 82.9231607241% along the line
```
...
```sh
potentia read --length 32 --mode scrambled 0ec971d8ace67261044340a270bfadb62f567ec   # "it was the best of times"
potentia read --line image --mode scrambled <address> --out found.png
potentia read --line audio --mode scrambled <address> --out tune.mid
potentia read --length 12 --mode positional --around 3 <address>
```
...
```sh
potentia browse --length 32
potentia browse --line image --count 3
potentia browse --line audio
```

### `sieve`: how much of the text space survives each noise filter (M1)
...
```sh
potentia sieve
potentia sieve --lengths 1-64,100,1000 --csv results/sieve.csv
potentia sieve --dict scowl-en-35 --lengths 1-20
...
Dictionaries are registered in `data/dictionaries/dictionaries.tsv`, one per line, with these fields: id, file, language, default, SHA-256 and description. Every command that takes `--dict ID` looks the id up there. If a file's hash no longer matches, the command refuses to use it, so a result recorded with a dictionary id can always be reproduced with exactly the same words. `potentia dicts` lists every entry, marks the default and checks each file.

To add a dictionary:

1. Put the word list (one word per line) in `data/dictionaries/`.
2. Run `potentia dicts --hash data/dictionaries/FILE`. It prints the word count, the SHA-256 and a ready-made registry line.
