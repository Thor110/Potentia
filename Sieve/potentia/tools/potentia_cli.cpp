// Potentia — command-line interface to the core.
//
...
//   potentia read   ... ADDRESS --around N      (the N units either side)
//   potentia dicts  [--hash FILE]
//   potentia version
//   potentia sieve  [--dict ID|PATH] [--lengths SPEC] [--brute-max N] [--pruned-max N] [--threads T] [--csv PATH]
//
// Run "potentia help", "potentia help <command>" or "potentia help lines" for explanations and examples.
...
#include "potentia/audio.hpp"
#include "potentia/image.hpp"
#include "potentia/sha256.hpp"
#include "potentia/sieve.hpp"
#include "potentia/utf8.hpp"
...
using namespace potentia;
using namespace potentia::cli;
...
int cmd_sieve(const Args& a)
{
    ...
    *csv << "# potentia sieve, alphabet lower27, dictionary " << rd.id << " sha256 " << dict.sha256() << "\n"
...
int cmd_version()
{
    std::cout << "potentia " << POTENTIA_VERSION << "  (specification 2.0)\n\n"
...
int cmd_dicts(const Args& a)
{
    ...
    std::cout << "\n* = default. Use one with --dict ID. To add a dictionary, see: potentia help dicts\n";
    return 0;
}

bool is_command(const std::string& name)
{
    return name == "info" || name == "warp" || name == "read" || name == "browse" || name == "sieve" || name == "dicts" ||
           name == "version";
}
...
int main(int argc, char** argv)
{
    try
    {
        ...
        if (a.help && print_help(a.command)) return 0;
        ...
        if (a.command == "sieve") return cmd_sieve(a);
    ...
    catch (const std::exception& e)
    {
        ...
        if (argc >= 2 && is_command(argv[1])) std::cerr << "(see: potentia help " << argv[1] << ")\n";
        return 1;
    }
}
