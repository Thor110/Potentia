...
void book_face(float tile_z, Side side, int row, int col, Vec3 out[4])
{
    ...
    const float y1 = y0 + book_height(row, col);
    const float z0 = tile_z + col * kBookPitch + (kBookPitch - kBookWidth) * 0.5f;
    const float z1 = z0 + kBookWidth;
    out[0] = {x, y0, z0};
    out[1] = {x, y0, z1};
    out[2] = {x, y1, z1};
    out[3] = {x, y1, z0};
...
std::optional<BookSlot> pick_book(Vec3 origin, Vec3 dir, int64_t player_tile, float reach)
{
    ...
    const int col = int(zin / kBookPitch);
    const float within = zin - col * kBookPitch - (kBookPitch - kBookWidth) * 0.5f;
    if (within < 0 || within > kBookWidth) return std::nullopt;

    const float from_top = kRowTop - hit.y;
    if (from_top < 0) return std::nullopt;
    const int row = int(from_top / kRowHeight);
    if (row < 0 || row >= kRows || col < 0 || col >= kCols) return std::nullopt;
    const float y0 = kRowTop - (row + 1) * kRowHeight + 0.02f;
    if (hit.y < y0 || hit.y > y0 + book_height(row, col)) return std::nullopt;
