// Potentia — a text line's address space in raw mode (SPECIFICATIONS §3, §4.1).
//
...
public:
    ...
    Space(const Alphabet& alphabet, uint32_t unit_length, std::string key = "potentia");
    // A space over any symbol set, identified by name and size (images, audio, video).
    Space(std::string symbols_id, uint32_t base, uint32_t unit_length, std::string key = "potentia");
    ...
    // Parameter-set identifier, e.g. "lower27/L32/key=potentia/feistel-sha256-v1".
