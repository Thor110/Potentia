...
- When lengths differ, the mapping is many-to-one in one direction. The client therefore records the **return path**: stepping back through the door the reader just used always returns them to their exact origin.
...
1. **Reference implementation** of the core in a high-level language. Clarity over speed. This is the source of truth.
2. **Conformance suite:** fixed inputs with expected addresses, classifications, door mappings and canonical forms, checked bit for bit.
3. **Fast core** in C (SIMD where profiling justifies it; assembly only for proven hot spots). It must pass the full conformance suite before it is trusted.
4. **Models as pluggable scorers**, run through existing GPU libraries, with quantised integer outputs passed to the core.
...
| :--- | :--- | :--- |
| M1 | Exhaustive sieve | Enumerator + S0/S1 filters over **every** unit: text at lengths 4–8, images at 5×5 and 6×6 1-bit; plot of surviving fraction vs. size |
| M2 | Raw addressing and warp | Positional and scrambled bijections, canonicalisation rules, CLI warp, round-trip tests |
| M3 | Entropy-ordered addressing | Integer arithmetic coder with a small pinned text model; measured bits/char on real text |
| M4 | Hallway prototype | 2D side view: shelves, zoom depth, readout, warp box, in-hand view, guided/raw toggle |
| M5 | Doors | Image and symbolic audio lines; fractional door mapping with return paths |
| M6 | Anchor Registry | Local, signed, append-only Registry with review workflow; anchored units shelved and marked |
| M7 | Paragraph-scale sampling | Classification and sampling at `UNIT_LENGTH` ≈ 1,000; extrapolation checked against M1 |
| M8 | 3D hallway | 3D client over the same core interface |
